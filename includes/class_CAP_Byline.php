...
class CAP_Byline
{
  ...
  private function load_dependencies()
  {
    ...
    if($this->subclasses_loaded["loader"]) {
      $this->loader = new CAP_Byline_Loader(array("debug" => $this->debug));
    }
  }
  ...
  private function set_locale()
  {
    if($this->subclasses_loaded["i18n"]) {
      $plugin_i18n = new CAP_Byline_i18n(array("debug" => $this->debug));
  ...
  private function define_admin_hooks()
  {
    if($this->subclasses_loaded["admin"]) {
      $plugin_admin = new CAP_Byline_Admin(array("debug" => $this->debug, "plugin_name" => $this->plugin_name, "version" => $this->version, "plugin_core" => $this));
  ...
  private function define_public_hooks()
  {
    if($this->subclasses_loaded["public"]) {
      $plugin_public = new CAP_Byline_Public(array("debug" => $this->debug, "plugin_name" => $this->plugin_name, "version" => $this->version, "plugin_core" => $this));
  ...
  private function register_taxonomies()
  {
    ...
    if(   isset($CAP_Byline_taxonomies)
       && is_array($CAP_Byline_taxonomies)
       && count(array_keys($CAP_Byline_taxonomies)) > 0) {
  ...
  private function register_acfs()
  {
    ...
    if(   isset($CAP_Byline_acfs)
       && is_array($CAP_Byline_acfs)
       && count(array_keys($CAP_Byline_acfs)) > 0) {
  ...
  private function set_terms($post_id)
  {
    ...
    # TODO: Setup a configurable list of posts for which we should not set terms.
    ...
    if(empty($field_data)) {
      ...
      if(   !empty($default_byline_override)
         && false === get_field( 'disable_auto_author_select','options')) {
        $persons[] = $default_byline_id;

      // If a person exists with the slug of the author then auto add it.
      } elseif(   term_exists($author_slug, 'person')
               && false === get_field('disable_auto_author_select','options')) {
        $persons[] = $author_id;
      }
  ...
  public function get_cap_authors($post_id, $disable_link=false, $as_array=false, $return_slugs=true, $byline_field='byline_array')
  {
    ...
    // Check for the display function, if as_array is set to true then just
    // return the array... if not then proceed with the listing function.
    if(true == $as_array && true == $return_slugs) {
      return $byline_array;
    }
    ...
    if(!empty($byline_array)) {
      foreach($byline_array as $author) {
        ...
        # We're past the first person which means we need to join the names
        # together.
        if($i > 0) {
          # If we only have two people, use an ampersand to join them.
          ...
          # We have more than two people total so we'll use commas instead.
          } else {
            $output[] = ', ';
          }
        }
   ...
   * @since    1.0.0
   *
   * @param $type defines the types of byline available.
   * @param $post_id is the post id, we should pass this in always.
   **/
  private function generate_timestamp($post_id)
  {
    ...
    if(   is_singular()
       && true === get_field('global_display_post_time', 'options')) {
      $time_format = 'F j, Y \a\t g:i a';
    } else {
      $time_format = 'F j, Y';
    }

    $time_string = array('<time class="published" datetime="%1$s">%2$s</time>');
    ...
    return sprintf(implode('', $time_string),
   ...
   * @since    1.0.0
   *
   * @param $post_id is the post id.  We'll need this to pull all post authors.
   * @param $time_string is the formatted date/timestamp.
   **/
   ...
   * @since    1.0.0
   *
   * @param $time_string is the formatted last update timestamp.
   **/
   ...
   * @since    1.0.0
   *
   * @param $style defines the types of byline available.
   * @param $post_id is the post id, we should pass this in always.
   **/
